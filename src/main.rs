...
impl<'a> Contains for Manufacturer<'a> {
    fn contains(&self, needle: &str) -> bool {
        self.name.unwrap_or_default().to_lowercase().contains(needle)
            || self.common_name.unwrap_or_default().to_lowercase().contains(needle)
            || self.country.unwrap_or_default().to_lowercase().contains(needle)
    }
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let results = res.as_object()
        ...
        .unwrap()
        .iter();
    let manufacturers = results.map(|manufacturer_json| {
